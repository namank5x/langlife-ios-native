...
enum LocalVoiceSessionLimitStore {
    ...
    static func loadCount(userId: UUID?, dayStart: Date) -> Int {
        ...
        return AppDatabase.read { db in
            let row = try Row.fetchOne(
                db,
                sql: "SELECT count FROM voice_session_limits WHERE user_id = ? AND day_start = ?",
    ...
    static func incrementCount(userId: UUID?, dayStart: Date) -> Int {
        ...
        AppDatabase.write { db in
            ...
            try db.execute(
                sql: """
                INSERT INTO voice_session_limits (user_id, day_start, count)
                VALUES (?, ?, ?)
